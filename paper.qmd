...
**Elitism and convergence.** The King's fitness never increases: it is replaced only by strictly better points, whether through the population update, en passant, or castling. That elitism buys the usual asymptotic guarantee.
...
The suite consists of six standard functions covering unimodal, ill-conditioned, and highly multimodal terrain:
...
One-way ANOVA confirms that the algorithm factor is highly significant on every function:

{{< include results/table_anova.md >}}

: One-way ANOVA over the five algorithms, per function. {#tbl-anova}

The pairwise Wilcoxon tests give the sharper picture:
...
The best plan discovered (by CA) selects the minimum cycle, $C = 60$ s — a sensible choice under Webster's guidance whenever splits can be kept efficient — with the following timings:
...
<!-- SECTION CONTENT ADDED AFTER EXPERIMENTS COMPLETE -->
...
<!-- CONCLUSION MEALPY SENTENCE ADDED AFTER EXPERIMENTS -->
