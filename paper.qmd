...
@fig-motivation states the argument of this section in one picture: the structural properties of these objectives on the left, the two features of a conventional homogeneous population that answer them only partially in the middle, and the two design decisions with which the algorithm proposed here responds on the right. The remainder of the introduction develops each column in turn.

![Motivation and conceptual architecture. The objectives that arise in transportation and engineering design are non-convex, multimodal and frequently expensive to evaluate (left). A homogeneous population answers them with a single update rule applied to every agent, and schedules its exploration–exploitation transition on elapsed time alone (centre). The Chess Algorithm answers the first property with heterogeneous roles assigned by rank and the second with a controller that reads the observed state of the population each iteration (right). The sketched landscape is generated analytically and carries no experimental data.](figures/concept_motivation.png){#fig-motivation fig-align="center" width="100%"}
...
The Chess Algorithm (CA) proposed here formalizes that mapping. @fig-abstraction states it precisely: each piece becomes one movement geometry around the incumbent solution, and the figure names, for each, the search behaviour it produces and whether that behaviour contributes primarily to exploration or to exploitation. The right-hand column of that figure is deliberately not a claim of novelty — @sec-operator-families maps every one of these operators onto an established family with its canonical reference.

![The chess abstraction, stated as a mapping onto search operators. Each piece contributes one movement geometry around the incumbent King, sketched from the equation given in the third column; the last column records whether the role contributes primarily to exploration or to exploitation. No operator here is new in kind: @tbl-operator-families names the established family and canonical reference of each, and the metaphor's contribution is the composition, not any individual operator.](figures/concept_abstraction.png){#fig-abstraction fig-align="center" width="94%"}

To preempt a common misunderstanding: CA does not play chess and does not search a game tree. It borrows the strategic grammar of the game exclusively, and its per-iteration computational cost is comparable to that of GA or PSO.
...
Before the components are defined one by one, @fig-architecture shows how they are assembled into a single iteration. Three strands run in parallel. The population is ranked, moved by role-specific operators, evaluated, and filtered by an acceptance rule; the King runs its own local search through en passant, castling, and the tactical probes; and the adaptive controller of @sec-adaptive reads four statistics of the population and conditions the rates at which the tactical operators fire. Solid edges carry candidate solutions, dashed edges carry control signals. Each box names the equation or section in which it is specified, so the figure doubles as a map of the rest of this section.

![One iteration of CA. Ranking assigns roles, roles move under their own geometry, the King runs its own local search, and the adaptive controller reads the divergence, initiative, local-search success and stagnation of the population and conditions which tactical operators fire and at what rate. Solid arrows carry the population and the incumbent; dashed arrows are control signals. The complete step is given as pseudocode in @sec-ca.](figures/ca_architecture.png){#fig-architecture fig-align="center" width="100%"}
...
![Heterogeneous role-based search. (a) Ranking partitions the population each iteration in the fixed proportions of @tbl-params, the Pawns taking the remainder. (b)–(f) The geometry of each role operator in a two-dimensional section: blue is the incumbent King, grey the moving agent, teal a better-ranked peer, and orange the candidates the operator produces. The candidate clouds are not drawn by hand — they are 45 draws from the equation named in each panel title, evaluated with a fixed seed. The Rook moves one coordinate and the Bishop two of equal magnitude, so both produce axis-structured candidates; the Queen sweeps in every direction at a radius tied to its distance from the King; the Knight combines a drift toward a better-ranked peer with an asymmetric jump; the Pawn takes small steps toward both attractors, which is why its cloud is the tightest.](figures/ca_roles.png){#fig-roles fig-align="center" width="100%"}
...
The correspondence between this pseudocode and @fig-architecture is one to one: lines 5–6 are the controller strand, lines 7–11 the population strand, and lines 12–16 the King's local search.
...
The mechanisms above give CA its movement vocabulary; what remains is to say how CA decides, moment to moment, which of them to lean on. A grandmaster does not run the same combination of tactics in a wide-open middlegame as in a locked pawn structure or a simplified endgame — which tactic is right depends on the position, not on a move counter. CA answers this with a lightweight state machine that reads four cheap statistics of the population each iteration and switches the active tactical repertoire accordingly, described below and summarized in @fig-statemachine.

![The adaptive tactical controller. Four smoothed statistics — divergence $\tilde d$ (@eq-divergence), initiative $\alpha$ (@eq-acceptance), en-passant success rate $\varepsilon$ (@eq-epsilon), and the fifty-move stagnation counter $s$ (@eq-fifty-move) — are read once per iteration. Divergence and the elapsed fraction of the budget select one of four phases under the thresholds of @eq-phase, and each phase activates the operator rates reproduced in the lower panel from @tbl-phase-schedule. Two overlays act independently of the phase: the zugzwang rule of @eq-zugzwang and the blockade response of @sec-blockade. All printed values are those of @tbl-phase-schedule; the shading is proportional to the printed rate and carries no additional information.](figures/ca_statemachine.png){#fig-statemachine fig-align="center" width="100%"}
...
The five operators introduced below are what the phases of @tbl-phase-schedule switch between. @fig-tactics shows where each of them places a candidate relative to the incumbent and the elite, which is the property that distinguishes them from one another and from the role moves of @fig-roles: the en-passant trial is isotropic around the King at an adaptively scaled radius, the fork inherits a difference vector between two better-ranked peers, the council reflects away from the elite centroid, the interference recombines two elite members along the segment joining them, and the discovered attack extrapolates through the incumbent.

![Geometry of the five tactical operators, each in a two-dimensional section, with points produced by the equation named in the panel title. (a) En passant: three masked Gaussian trials at the adaptive radius $\boldsymbol{\sigma}$, which is multiplied by $1.3$ after a successful capture and by $0.92$ after a failure. (b) Knight's fork: a drift toward the King plus the difference vector of two better-ranked peers. (c) Royal council: the King reflected away from the midpoint of the best Queen and best Rook. (d) Novotny interference: convex recombination of two members of the fitter half, which makes it rotation-invariant. (e) Discovered attack: extrapolation through the incumbent along an elite–King line, restricted to a single elitist probe.](figures/ca_tactics.png){#fig-tactics fig-align="center" width="100%"}
...
@fig-cec2017 shows median convergence on six representative functions and @fig-cec2017-dist the distribution of final errors on the same six. The six were chosen to span the four official categories — one unimodal, one multimodal, two hybrid, two composition — and are not selected for CA's advantage: they include F4 and F22, on which both competition-grade methods beat CA.

![Median convergence on six representative CEC-2017 functions, one or more per official category (30 runs, semi-log scale). The same six functions are used in @fig-cec2017-dist.](figures/cec2017_convergence.png){#fig-cec2017 fig-align="center" width="100%"}

![Distribution of the final error over 30 runs on the six representative CEC-2017 functions of @fig-cec2017 (log scale; box = interquartile range, whiskers = 1.5 IQR, points = outliers). The figure answers a question the means alone cannot: whether a ranking is carried by typical runs or by outliers. The picture is not uniform. On F1 the distributions of CA, CA-static, L-SHADE and CMA-ES overlap almost entirely — CA holds the lowest mean of the roster on that function, and its comparisons against both competition-grade methods are statistical ties ($p=0.21$ against each) — whereas on F4, F10 and F13 the two sit roughly an order of magnitude below CA, with tighter distributions, and beat it significantly. On the two composition functions the absolute gaps are small but consistent across runs, and CA loses to both methods there as well.](figures/cec2017_boxplots.png){#fig-cec2017-dist fig-align="center" width="100%"}
...
@fig-cec2022 shows median convergence on three representative cells per dimensionality.

![Median convergence on six representative CEC-2022 cells — F1 (Zakharov), F6 (Hybrid 1), and F9 (Composition 1) at each of the two official dimensionalities (30 runs, semi-log scale). The three functions span the suite's basic, hybrid, and composition classes, and they are not selected for CA's advantage: on F9 CA holds the best mean at $D=10$ and ties both competition-grade methods there, but loses to both significantly at $D=20$.](figures/cec2022_convergence.png){#fig-cec2022 fig-align="center" width="100%"}

## Discussion

The ordering at the top of @tbl-cec2022-summary is unambiguous and consistent with @sec-cec2017: the two competition-grade methods lead (L-SHADE $1.88$, CMA-ES $2.06$), followed by CA ($3.63$) and CA-static ($3.75$) as the best-ranked of the remaining six, then GA ($4.75$), PSO ($5.63$), GWO ($6.44$), and WOA ($7.88$). The pattern is stable across dimensionalities: restricting the Friedman analysis to the eight $D=10$ cells gives L-SHADE first and CMA-ES second ($\chi^2=39.3$, $p=1.7\times10^{-6}$), and restricting it to the eight $D=20$ cells gives CMA-ES first and L-SHADE second ($\chi^2=44.1$, $p=2.0\times10^{-7}$), with CA third at $D=20$ and effectively tied with CA-static for third at $D=10$.
...
![Mechanism contribution. (a) For each of CA's twelve mechanisms, the bar is the ratio of the mean final error with that mechanism disabled to the mean final error of full CA, averaged over the six problems of this section (log scale; above 1 means the ablated variant is worse, i.e. the mechanism helps), with the number of problems on which the Wilcoxon difference is significant at $\alpha=0.05$ in parentheses. (b) The same ratio per problem for the two load-bearing mechanisms. The metric is a ratio of means under one-at-a-time removal, not an importance score: it is defined only relative to full CA, and mechanisms with overlapping function can each appear individually dispensable even when they are not collectively so.](figures/ablation_contribution.png){#fig-ablation fig-align="center" width="100%"}
...
A one-at-a-time sensitivity analysis perturbs each parameter group of @tbl-params around the published default configuration — role fractions bracketed low and high, $\theta_0 \in \{0.05, 0.2\}$, castling period $c \in \{5, 20\}$, blockade threshold $\in \{15, 40\}$, and pinning cap $\in \{0.15, 0.50\}$ — on the same six problems and run protocol as @sec-ablation. @tbl-sensitivity reports each variant's mean-error ratio relative to the default configuration, and @fig-sensitivity shows the same quantity with its per-problem range. The phase thresholds $\delta_{\text{open}}, \delta_{\text{end}}$ of @eq-phase are not re-swept here; the four-combination check reported in @sec-adaptive stands.
...
![Parameter sensitivity profile. Each row perturbs one parameter group of @tbl-params around the published default with all others held fixed; the marker is the mean final-error ratio of the perturbed configuration to the default over the six problems of @sec-ablation, and the line spans the per-problem range. Filled markers mark variants with at least one statistically significant per-problem deviation — only the two role-fraction rows qualify. Because the design is one-at-a-time, these are local sensitivities around a single configuration and not a global sensitivity analysis: interactions between parameters are not measured.](figures/sensitivity_profile.png){#fig-sensitivity fig-align="center" width="90%"}
...
Cost is measured, not estimated: an exact counting wrapper intercepts every objective call, and wall-clock time is taken with a monotonic timer over ten runs per cell on an otherwise idle machine, for all eight algorithms on six problems spanning the paper's sections (CEC-2017 F1, F13, F22 at $D=30$; CEC-2022 F8 at $D=20$; welded beam; signal timing P1 at its own $T=300$ protocol). @tbl-timing reports mean seconds per run for each algorithm on each of the six problems, together with the mean ratio of evaluations consumed to the $N \times T$ core budget and the mean time relative to GA; @tbl-app-timing gives the per-problem evaluation counts behind those two averages.

{{< include results/table_t_timing.md >}}

: Measured wall-clock time (mean seconds per run, ten runs per cell on an idle machine) with the algorithms as rows and the six timed problems as columns, followed by two summary columns averaged over those problems. "Evals / core budget" is the ratio of consumed evaluations to the shared core budget $N \times T$, averaged over the six problems and counted exactly by a wrapper intercepting every objective call; it exceeds $1$ slightly for GWO, PSO, GA and WOA because the initial population evaluation lies outside $N \times T$, and falls below $1$ for CMA-ES only because it stops early on the welded beam ($60\%$ of the budget), consuming exactly the core budget on the other five problems. The per-problem evaluation counts behind these ratios are in @tbl-app-timing. {#tbl-timing}
...
The three suites of @sec-cec2017, @sec-cec2022, and @sec-engineering were run under one roster and one protocol, so their Friedman ranks can be read side by side. @fig-standing does that, and it is the compact form of this paper's central empirical claim. The ordering is the same on all three suites and admits no qualification: L-SHADE and CMA-ES lead, CA follows as the best of the remaining six, and CA-static sits just behind it. CA's position is therefore neither a sweep nor a failure, and the ablation of @sec-ablation explains why it is where it is — CA's two load-bearing operators are fixed-rate members of exactly the two families those two methods specialize in and adapt fully.

![Where CA stands. Mean Friedman rank of each algorithm (lower is better) on the three suites evaluated under the identical eight-algorithm roster and protocol. The ordering is stable across all three: the two competition-grade methods lead, CA is the best-ranked of the remaining six on every suite, and CA-static follows it. The classical six-function suite of @sec-benchmarks is not shown, because it was run under a different roster — it includes SA and excludes WOA, L-SHADE, and CMA-ES.](figures/standing_ranks.png){#fig-standing fig-align="center" width="94%"}

Two qualifications belong with the figure. It compares mean ranks, which compress the per-function records of @tbl-cec2017-summary, @tbl-cec2022-summary, and @tbl-engineering-summary — CA's ten losses to GA on CEC-2017 are invisible in a mean rank that GA nonetheless trails. And a rank is relative to the roster: adding or removing an algorithm changes every rank in the figure, which is why the pairwise Wilcoxon records are reported alongside it throughout the paper rather than in place of it.
...
with a common cycle length $C \in [60, 140]$ s, arterial green splits $g_i \in [0.30, 0.75]$, and offsets $o_i$ expressed as fractions of the cycle (intersection 1 serves as reference). @fig-signal-model shows which physical object each of the sixteen variables controls, and why the coupling between the offsets, the spacings, and the progression speed is what makes this landscape multimodal. The objective is total network delay in veh·h/h, built from three ingredients.

![From the arterial to the decision vector. (a) The eight intersections share one cycle length $C$; each contributes an arterial green split $g_i$, and each except the reference intersection an offset $o_i$ — the sixteen variables of @eq-decision. (b) The offsets displace each local green window in time; a platoon that leaves one window inside the green arrives at the next either inside its window (green, no delay) or after it (orange, and it waits). Because the arterial carries platoons in both directions, offsets that perfect one direction damage the other, which is the origin of the multimodality. Panel (b) is a schematic at an illustrative cycle and split; the optimized plan is @fig-plan.](figures/signal_model.png){#fig-signal-model fig-align="center" width="100%"}
...
@tbl-traffic summarizes the final delays together with the pairwise tests; @fig-traffic-conv and @fig-traffic-box show the convergence behavior and the spread across runs.
...
![Time–space diagram of the best berth allocation plan found by CA. Each rectangle is one ship: its width is the required service time and its height the ship's length, so a feasible plan is one in which no two rectangles overlap. Dotted lines mark arrival times and the orange arrows the waiting time between arrival and mooring, which is what separates this plan from the optimum. The header states the plan's objective, the known global optimum $F^{*}=4{,}860$ min, the resulting optimality gap, and the residual overlap — zero, so the plan is feasible. The gap is therefore waiting time alone, not infeasibility.](figures/berth_best_plan.png){#fig-berth-plan fig-align="center" width="92%"}
