...
ALGO_TEX = {"CA": "CA", "CA-static": "CA-static", "GWO": "GWO", "PSO": "PSO",
            "GA": "GA", "WOA": "WOA", "L-SHADE": "L-SHADE", "CMA-ES": "CMA-ES"}
DISPLAY_NAME = {
...
def _excluded_note(present_funcs, full_range=range(1, 30)):
    excluded = sorted(set(full_range) - {fnum(f) for f in present_funcs})
    if not excluded:
        return ("All 29 usable functions are included (six of them "
                "restored from the independent official-data "
                "implementation after the audit).")
    names = ", ".join(f"F{n}" for n in excluded)
    plural = "is" if len(excluded) == 1 else "are"
    return f"{names} {plural} excluded."
...
def cec_detail_longtable(stats_csv, wilc_csv):
    ...
    lines = [
        ...
        rf"\caption{{CEC-2017 results (D=30): mean error $f-f^{{*}}$ over "
        ...
        rf"worse, $^{{=}}$ not significant. {_excluded_note(funcs)} "
        r"(see Section~\ref{sec:opfunu-audit}).}}",
...
def main():
    cec_stats = pd.read_csv(os.path.join(RESULTS, "cec2017_stats.csv"))
    n_cec_funcs = cec_stats["func"].nunique()
    ...
    parts = [
        ...
        f"% ---- CEC-2017 ({n_cec_funcs} valid functions; see the "
        "data-integrity\n"
        "%      audit for the excluded function(s)) ----\n",
        rank_table(os.path.join(RESULTS, "cec2017_mean_ranks.csv"),
                  f"CEC-2017 (D=30, {n_cec_funcs} valid functions): mean "
                  "Friedman rank over 30 independent runs per function "
                  "(lower is better).",
        ...
        wtl_table(os.path.join(RESULTS, "cec2017_wilcoxon.csv"), "func",
                 ...
                 f"(Wilcoxon rank-sum, $\\alpha=0.05$, {n_cec_funcs} "
                 "functions).",
                 "tab:cec2017-wtl"),
